...
class User
{
  ...
  // Nuevo método para ingresar con Google
  ...
  private function saveWithGoogle()
  {
    $sql = "INSERT INTO usuarios (nombre, apellidos, email, rol, imagen) VALUES (?, ?, ?, 'user', ?);";
    try {
      $stmt = $this->database->prepare($sql);
      $stmt->bind_param("ssss", $this->name, $this->surnames, $this->email, $this->image);
      $stmt->execute();
      $stmt->close();
      return true;
    } catch (\Throwable $th) {
  ...
  private function getOneByEmail()
  {
    $sql = "SELECT * FROM usuarios WHERE email = ?;";
    $stmt = $this->database->prepare($sql);
    $stmt->bind_param("s", $this->email);
